...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 15:31:50 2025
...
def load_woe_data():
    """Loads the corrected WOE-transformed train and test datasets."""
    print("Loading corrected WOE datasets...")
    # This assumes a new WOE transformation was run on the corrected featured data.
    # We will name the files `train_woe_corrected.csv` and `test_woe_corrected.csv`.
    try:
        train_df = pd.read_csv('train_woe.csv')
        test_df = pd.read_csv('test_woe.csv')
        
        # Ensure target is in 0/1 format, just in case
        ...
        print("Please ensure you have run the WOE binning script on the '..._featured_corrected.csv' files to generate the WOE datasets.")
        return None, None
...
def plot_metrics_table(metrics_data, title='Final Logistic Regression Performance'):
    ...
    plt.savefig('final_lr_metrics_report.png', bbox_inches='tight', dpi=150)
    plt.show()
...
def plot_roc_pr_curves(y_train, y_train_proba, y_test, y_test_proba):
    ...
    plt.suptitle('Logistic Regression Evaluation Curves', fontsize=18)
    plt.savefig('final_lr_evaluation_curves.png', bbox_inches='tight', dpi=150)
    plt.show()
...
def plot_calibration_curve_and_thresholds(y_true, y_proba):
    ...
    ax1.plot(prob_pred, prob_true, marker='o', label='Logistic Regression')
    ...
    plt.suptitle('Logistic Regression Diagnostics (Test Set)', fontsize=18)
    plt.savefig('final_lr_model_diagnostics.png', bbox_inches='tight', dpi=150)
    plt.show()

def main():
    """Main function to train the final Logistic Regression model and evaluate its performance."""
    train_df, test_df = load_woe_data()
    if train_df is None or test_df is None: return

    # Define all valid predictors from the WOE data, excluding any leaky or target-related columns.
    # The `optbinning` transform replaces the original columns with their WOE values, keeping the same names.
    valid_predictors = [
        'age', 'job', 'marital', 'education', 'default', 'balance', 'housing', 
        'loan', 'contact', 'day', 'month', 'campaign', 'pdays', 'previous', 
        'poutcome', 'balance_to_age_ratio', 'total_liabilities', 
    ...
    print(f"\nTraining model with {len(final_feature_list)} WOE-transformed features.")
    ...
    print("\nTraining the final Logistic Regression model on the full WOE training dataset...")
    # Since data is preprocessed with WOE, we can use the classifier directly.
    final_model = LogisticRegression(class_weight='balanced', random_state=42, solver='liblinear')
    ...
    print("\n--- Final evaluation for Logistic Regression complete. All plots have been saved. ---")
