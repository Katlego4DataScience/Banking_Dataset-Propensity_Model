...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 15:10:50 2025
...
def plot_metrics_table(metrics_data, title='Final Gradient Boosting Performance'):
    ...
    plt.savefig('final_gb_metrics_report.png', bbox_inches='tight', dpi=150)
    plt.show()
...
def plot_roc_pr_curves(y_train, y_train_proba, y_test, y_test_proba):
    ...
    plt.suptitle('Gradient Boosting Evaluation Curves', fontsize=18)
    plt.savefig('final_gb_evaluation_curves.png', bbox_inches='tight', dpi=150)
    plt.show()
...
def plot_calibration_curve_and_thresholds(y_true, y_proba):
    ...
    ax1.plot(prob_pred, prob_true, marker='o', label='Gradient Boosting')
    ...
    plt.suptitle('Gradient Boosting Diagnostics (Test Set)', fontsize=18)
    plt.savefig('final_gb_model_diagnostics.png', bbox_inches='tight', dpi=150)
    plt.show()

def main():
    """Main function to train the final Gradient Boosting model and evaluate its performance."""
    ...
    print("\nTraining the final Gradient Boosting model on the full training dataset...")
    ...
    y_train_pred = final_pipeline.predict(X_train) # Using default 0.5 threshold
    y_test_pred = final_pipeline.predict(X_test)   # Using default 0.5 threshold
    ...
    print("\n--- Final evaluation complete. All plots have been saved. ---")
